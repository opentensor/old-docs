...
- The first assessment is the [consensus score](#consensus) which is determined by the number of approval votes given by validators ([trust](#trust)). This score is not earned in a linear fashion, as Validators with more stake ([Tao](#tao)) are able to distribute a greater number of votes. The score is also regularized by a sigmoid function: if less than 51% of the votes a Server receives are positive, the score is reduced exponentially, if more than 51% are positive, it is likewise inflated. 
